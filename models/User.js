// Function-based User model
const db = require('../db');

const User = {
  create({ username, email, password, address, contact, role }, callback) {
    // Use MySQL SHA1 hashing like original app for consistency with the dump
  ...
  findByEmail(email, callback) {
    const sql = 'SELECT id, username, email, address, contact, role FROM users WHERE email = ?';
